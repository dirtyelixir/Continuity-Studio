...
const state=createHandoffState();state.select('project','scene','shared');
assert(renderHandoff(project,state).includes('重新生成全域提示詞'));
for(const status of ['queued','running','awaiting_input']){
 project.scene_prompt_regenerations={scene:{job_id:'job',state:status}};
 const html=renderHandoff(project,state);assert(html.includes('正在重新生成…')&&/data-action="regenerate-scene-prompt"[^>]*disabled/.test(html));
}
project.scene_prompt_regenerations.scene={job_id:'job',state:'succeeded',stale:false};
assert(renderHandoff(project,state).includes('比較新版'));
project.scene_prompt_regenerations.scene.adopted=true;assert(renderHandoff(project,state).includes('已採用重新生成的版本'));
project.scene_prompt_regenerations.scene={job_id:'job',state:'failed',error:'<bad>'};
const html=renderHandoff(project,state);assert(html.includes('原文仍保留')&&html.includes('&lt;bad&gt;'));
console.log('Scene regeneration controls: generate, pending/manual states, review/adopt status, failure and escaping passed.');
